...
현재 폴더가 `work/example-contest-filter/`라면:
...
```powershell
python work/example-contest-filter/src/main.py
```

Notion 확장 구조만 확인하려면:

```powershell
python work/example-contest-filter/src/notion_upload_stub.py
```
...
```text
work/example-contest-filter/output/review_queue.md를 읽고, 공모전 후보를 유지/보류/제외로 검수해줘. 판단 근거가 부족한 항목은 추가 확인 질문으로 남겨줘.
```
